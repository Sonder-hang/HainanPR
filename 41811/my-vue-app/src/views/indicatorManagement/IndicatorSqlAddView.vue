          ...
          <template v-if="form.kind === 'core18'">
            ...
            <label class="block text-[12px]">
              ...
              <p class="mt-0.5 text-[11px] text-[#B8BCCC]">
                STRUCTURE 适用于子项有排行（如 ICD 编码分布），SQL 直接返回分组聚合结果，系统自动渲染排行榜图表。
              </p>
            </label>
          ...
          <div class="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div class="bg-white border border-[#b8c9e8]/60 rounded-[2px] p-4">
              ...
              <p class="text-[12px] text-[#596080] leading-relaxed">
                子项配置（JSON）仅用于 <strong>STRUCTURE（排行榜型）</strong>，用于定义 SQL 字段映射（ranking_key_field / ranking_value_field）。<strong>RATE（率型）</strong>无需配置此项，留空即可，系统自动根据分子/分母 SQL 计算比值。
              </p>
            </div>

            <!-- 两种模板类型与子项配置的关系 -->
            <div class="bg-white border border-[#b8c9e8]/60 rounded-[2px] p-4">
              <h3 class="font-bold text-[#1F264D] text-[13px] mb-2">两种模板类型一览</h3>
              <div class="space-y-2 text-[11px]">
                <div class="flex items-start gap-2">
                  <span class="shrink-0 w-20 text-[10px] font-semibold text-[#1F264D] bg-[#e8eef9] px-1.5 py-0.5 rounded">STRUCTURE</span>
                  <span class="text-[#596080]">排行榜型，<strong>需要配置子项（JSON）</strong>。SQL 直接返回分组聚合结果，系统根据配置渲染排行榜图表。</span>
                </div>
                <div class="flex items-start gap-2">
            ...
            <!-- 字段详解（STRUCTURE 专用） -->
            <div class="bg-white border border-[#b8c9e8]/60 rounded-[2px] p-4">
              <h3 class="font-bold text-[#1F264D] text-[13px] mb-3">字段详解（仅 STRUCTURE 需配置）</h3>
              <div class="space-y-2">
                <div class="grid grid-cols-[140px_1fr] gap-x-3 text-[11px]">
                  <span class="font-mono text-emerald-600 font-semibold">type</span>
                  <span class="text-[#596080]"><strong>必填</strong>，配置类型，固定值 <code class="font-mono text-[10px] bg-[#f0f4ff] px-1 rounded">"COMPOSITE_RANKING"</code></span>
                </div>
                <div class="border-t border-[#e8eef9]"></div>
                <div class="grid grid-cols-[140px_1fr] gap-x-3 text-[11px]">
                  <span class="font-mono text-emerald-600 font-semibold">ranking_key_field</span>
                  <span class="text-[#596080]">排行榜维度字段名，SQL 返回结果中该列的值将作为排行项的标签展示（列名须与 SELECT 的列名一致，大小写敏感）</span>
                </div>
                <div class="border-t border-[#e8eef9]"></div>
                <div class="grid grid-cols-[140px_1fr] gap-x-3 text-[11px]">
                  <span class="font-mono text-emerald-600 font-semibold">ranking_value_field</span>
                  <span class="text-[#596080]">排行榜数值字段名，SQL 返回结果中该列的值将作为排行项的计数量参与排序（列名须与 SELECT 的列名一致，大小写敏感）</span>
                </div>
                <div class="border-t border-[#e8eef9]"></div>
            ...
            <!-- SQL 返回格式要求 -->
            <div class="bg-white border border-[#b8c9e8]/60 rounded-[2px] p-4">
              <h3 class="font-bold text-[#1F264D] text-[13px] mb-2">完整配置示例（可直接复制修改）</h3>
              <div class="bg-sky-50 border border-sky-200 rounded-[2px] p-3">
                <p class="text-[12px] font-semibold text-sky-700 mb-1">STRUCTURE 配置示例</p>
                <div class="font-mono text-[11px] text-[#334155] mt-1 bg-white rounded border border-sky-200 p-2 leading-relaxed">
{&#10;  "type": "COMPOSITE_RANKING",&#10;  "limit": 20,&#10;  "ranking_key_field": "sub_category",&#10;  "ranking_value_field": "disease_cnt"&#10;}
                </div>
              </div>

              <h3 class="font-bold text-[#1F264D] text-[13px] mt-4 mb-2">SQL 返回格式要求</h3>
              <div class="space-y-2 text-[11px] text-[#596080]">
                <div class="bg-sky-50 border border-sky-200 rounded-[2px] p-3">
                  <p class="font-semibold text-sky-700 mb-1">SQL 应返回多行排行榜数据，列名须与配置中的字段一一对应</p>
                  <div class="font-mono text-[11px] text-[#334155] mt-1 bg-white rounded border border-sky-200 p-2">
SELECT&#10;  sub_category,            -- 排行维度（对应 ranking_key_field）&#10;  COUNT(*) AS disease_cnt   -- 排行数值（对应 ranking_value_field）&#10;FROM FACT_DIAG_RECORD&#10;WHERE ...&#10;GROUP BY sub_category&#10;ORDER BY disease_cnt DESC
                  </div>
            ...
            <div class="bg-white border border-[#b8c9e8]/60 rounded-[2px] p-4">
              ...
              <ul class="text-[11px] text-[#596080] space-y-1.5 leading-relaxed">
                <li class="flex items-start gap-2">
                  ...
                  <span>RATE 型无需配置子项（留空），系统自动根据分子 SQL / 分母 SQL 计算比值。</span>
                </li>
                <li class="flex items-start gap-2">
                  <span class="mt-0.5 w-1.5 h-1.5 rounded-full bg-red-400 shrink-0"></span>
                  <span>JSON 中的字段名（ranking_key_field / ranking_value_field 等）与 SQL 返回的列名必须完全一致，<strong>大小写敏感</strong>。</span>
                </li>
                <li class="flex items-start gap-2">
...
const form = ref({
  ...
  promptContent: '',
  templateType: '' as '' | 'RATE' | 'STRUCTURE',
  subitemConfig: '',
})
  ...
  try {
    ...
    if (form.value.kind === 'core18') {
      ...
      const payload: Record<string, unknown> = {
        ...
        involved_tables: [...new Set([...form.value.numInvolvedTables, ...form.value.denInvolvedTables])],
